...
@pytest.fixture
def svc():
    """Service isolado: não acessa o Supabase."""
    with patch("src.service.load", return_value=[]), \
         patch("src.service.save_new", return_value=1), \
         patch("src.service.update_taken"):
...
class TestMedicationService:
    ...
    def test_mark_as_taken_valid(self, svc):
        svc.medications = [Medication("Dipirona", "12:00", db_id=1)]
        svc.mark_as_taken(0)
